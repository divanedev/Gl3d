...
void UploadUniformMat4f (unsigned int ShaderId, const std::string& UniformName, const glm::mat4& Matrix) {
    int UniformLocation = glGetUniformLocation(ShaderId, UniformName.c_str());
    if (UniformLocation == -1) {
        std::cout << "Warning: uniform '" << UniformName << "' does'nt exist!" << std::endl;
    }
